...
export default function MaterialList({ t }: MaterialListProps) {
  ...
  const handleAddMaterial = async () => {
    setAddLoading(true);
    try {
      const res = await axiosInstance.post("/api/material", newMaterial);
      if (res.status < 200 || res.status >= 300)
        throw new Error(t("material.addFailed") || "Failed to add material");
      enqueueSnackbar(t("material.added") || "Material added successfully", {
        variant: "success",
        preventDuplicate: true,
        autoHideDuration: 2000,
      });
      setShowAdd(false);
      setNewMaterial({ name: "", category: "", description: "", unit: "" });
      reload();
    } catch (error) {
      console.error(error);
  ...
  const handleEditMaterial = async () => {
    ...
    setEditLoading(true);
    try {
      const res = await axiosInstance.put(`/api/material/${editMaterial.id}`, {
      ...
      if (res.status < 200 || res.status >= 300)
        throw new Error(t("material.updateFailed") || "Failed to update material");
      enqueueSnackbar(t("material.updated") || "Material updated successfully", {
        variant: "success",
        preventDuplicate: true,
        autoHideDuration: 2000,
      });
  ...
  const handleDeleteMaterial = async () => {
    ...
    try {
      const res = await axiosInstance.delete(`/api/material/${deleteId}`);
      if (res.status < 200 || res.status >= 300)
        throw new Error(t("material.deleteFailed") || "Failed to delete material");
      enqueueSnackbar(t("material.deleted") || "Material deleted successfully", {
        variant: "success",
        preventDuplicate: true,
        autoHideDuration: 2000,
      });
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-bold text-green-800">
          {t("material.management") || "Material Management"}
        </h2>
        <button
          type="button"
        ...
        <div className="flex-1">
          <div className="relative">
            <input
              ...
              placeholder={t("material.searchByName") || "Search by name..."}
              value={search}
              onChange={(e) => {
        ...
        <table className="w-full text-left table-fixed">
          <thead>
            <tr className="bg-green-50 text-green-800 font-semibold">
              ...
              <th className="px-4 w-1/6">{t("material.unit") || "Unit"}</th>
      ...
      <div className="flex gap-4 mt-6 mb-2">
        <div className="bg-green-100 rounded p-4 w-1/4">
          <div className="font-semibold text-green-800">
            {t("material.total") || "Total Materials"}
          </div>
      ...
      {totalPages > 1 && (
        <div className="flex justify-between items-center text-sm text-gray-600 mt-4">
          <span>
            ...
            {t("material.outOf") || "out of"} {total}{" "}
            {t("material.materials") || "materials"}
          </span>
          <div className="flex gap-2">
            {page > 1 && (
              <button
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white rounded shadow p-6 w-96">
            ...
            <input
              className="border rounded px-2 py-1 w-full mb-3"
              placeholder={t("material.unit") || "Unit"}
              ...
              <button
                ...
                onClick={() => {
                  if (
                    ...
                    enqueueSnackbar(
                      t("material.fillAllFields") || "Please fill all fields",
                      {
                        variant: "error",
                        preventDuplicate: true,
                        autoHideDuration: 2000,
