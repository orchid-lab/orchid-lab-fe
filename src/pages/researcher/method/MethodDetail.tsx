  ...
  return (
    <main className="ml-64 mt-16 min-h-[calc(100vh-64px)] bg-gray-100 p-6">
      ...
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-[#005792] mb-2">{data.name}</h1>
        <p className="text-blue-700">{data.description}</p>
      </div>

      {/* Method Info Card */}
      <div className="bg-white/80 backdrop-blur-sm shadow-sm border border-blue-100 rounded-2xl p-6 mb-6">
      ...
      <div className="space-y-4">
        ...
        {data.methodStages.length === 0 ? (
          ...
          data.methodStages.map((stage) => (
            <div key={stage.id} className="bg-[#F0F8FF] rounded-2xl shadow-sm border border-blue-100 overflow-hidden">
              {/* Stage Header */}
              <div className="bg-blue-50/50 border-b border-blue-100 p-5">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-[#005792] text-white font-bold text-sm">
                        {stage.order}
                      </span>
                      <h3 className="text-lg font-bold text-[#005792]">
                        {stage.stageDefinition.name}
                      </h3>
                    </div>
                    <p className="text-sm text-blue-800 ml-11">{stage.stageDefinition.description}</p>
                  </div>
                  <div className="flex flex-col items-end gap-2 ml-4">
                    <span className="px-3 py-1 rounded-full text-xs font-medium bg-[#00CED1]/15 text-[#005792]">
              ...
              {/* Stage Content */}
              <div className="p-6 space-y-6">                {/* Materials Section */}
                <div>
                  ...
                  {stage.stageMaterials.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {stage.stageMaterials.map((item) => (
                        <div
                          key={item.id}
                          className="flex items-start gap-3 p-4 rounded-xl bg-white border border-blue-50 shadow-sm hover:border-blue-200 transition-colors"
                        >
                          <span className="inline-block w-2 h-2 rounded-full bg-[#00CED1] mt-1.5 flex-shrink-0"></span>
                          <div className="flex-1 min-w-0">
                            <div className="text-sm font-medium text-[#005792] truncate">
                              {item.material.name}
                            </div>
                            <div className="text-xs text-blue-600">{item.material.category}</div>
                          </div>
                        </div>
                      ))}
                    </div>
                      ...
                      {stage.stageChemicals.map((item) => (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-xs text-blue-600">{item.chemical.category}</div>
                          </div>
                        </div>
                      ))}
                    </div>
