  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{ border: "1.5px solid #e0edfa" }}
      >
        {/* Modal Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-blue-50 bg-gradient-to-r from-[#005792]/5 to-[#00CED1]/5 flex-shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <h2 className="font-bold text-[#005792] text-base leading-tight truncate">{method.name}</h2>
            </div>
          </div>
          <div className="flex items-center gap-3 flex-shrink-0 ml-3">
            {data && (
              <span className="hidden sm:flex items-center gap-1.5 text-xs font-semibold text-red-600 bg-red-50 border border-red-200 rounded-full px-3 py-1">
          ...
          {loading ? (
            <div className="space-y-3">
              {Array.from({ length: 3 }).map((_, i) => (
                <div key={i} className="border border-blue-100 rounded-xl p-4 animate-pulse space-y-3">
                  <div className="flex justify-between">
                    <div className="h-4 bg-blue-100 rounded w-1/2" />
                    <div className="h-5 bg-red-50 rounded-full w-20" />
                  </div>
              ...
              >
                {data.items.map((exp, idx) => (
                  <div
                    ...
                    className="border border-blue-100 rounded-xl p-4 hover:border-red-200 hover:shadow-sm transition-all bg-white group"
                  >
                    {/* Row 1: Name + Status */}
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <h3 className="font-semibold text-[#005792] text-sm leading-snug group-hover:text-[#004070] transition-colors">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 text-xs mb-3">
                      <div className="flex gap-1.5">
                        <span className="text-slate-400 flex-shrink-0">
                          {t("experimentLog.stage", "Giai đoạn thất bại")}:
                        </span>
                        <span className="font-medium text-orange-600">
                          #{exp.failedAtStageOrder} — {exp.failedAtStageName}
                        </span>
                      </div>
                      <div className="flex gap-1.5">
                        <span className="text-slate-400 flex-shrink-0">
                          {t("experimentLog.failedDate", "Ngày thất bại")}:
                        </span>
                        <span className="font-medium text-slate-700">{exp.failedDate}</span>
                      </div>
                      <div className="flex gap-1.5">
                        <span className="text-slate-400 flex-shrink-0">
                          {t("experimentLog.reason", "Lý do")}:
                        </span>
                        <span className="font-medium text-slate-700 line-clamp-1">{exp.reason}</span>
                      </div>
                      <div className="flex gap-1.5">
                        <span className="text-slate-400 flex-shrink-0">
                          {t("experimentLog.issues", "Vấn đề")}:
                        </span>
                    ...
                    {exp.recommendations && exp.recommendations !== "No recommendations" && (
                      <div className="bg-blue-50 rounded-lg px-3 py-2 flex gap-2 items-start">
                        <svg className="w-3.5 h-3.5 text-[#005792] flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                        </svg>
                        <p className="text-xs text-[#005792] leading-relaxed">{exp.recommendations}</p>
                      </div>
                    )}
                  </div>
                ))}
        ...
        {data && totalPages > 1 && (
          <div className="flex items-center justify-between px-6 py-3 border-t border-blue-50 bg-slate-50/50 flex-shrink-0">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="w-8 h-8 flex items-center justify-center rounded-lg border border-blue-100 hover:bg-blue-50 disabled:opacity-40 disabled:cursor-not-allowed text-[#005792] transition-colors"
              >
                      ...
                      <button
                        ...
                        className={`w-7 h-7 text-xs rounded-lg font-medium transition-colors ${
                          page === item
                            ...
                            : "border border-blue-100 text-[#005792] hover:bg-blue-50"
                        }`}
                      >
                        {(item as number) + 1}
                      </button>
...
function MethodCard({
  ...
  return (
    ...
    >
      ...
      {/* View Failed Experiments Button */}
      {method.failedExperimentLog > 0 && (
        <button
          type="button"
          onClick={() => onViewFailed(method)}
          className="w-full flex items-center justify-center gap-2 text-xs font-semibold text-red-600 bg-red-50 hover:bg-red-100 border border-red-200 rounded-xl py-2 transition-colors group"
        >
...
export default function ResearcherMethodSuccessRate() {
  ...
  const [selectedMethod, setSelectedMethod] = useState<MethodSuccessRate | null>(null);
