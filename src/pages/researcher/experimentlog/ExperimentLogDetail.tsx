...
const ExperimentLogDetail = () => {
  ...
  const handleChangeStage = async () => {
    if (!id || selectedBatchId === "") { setChangeStageError("Vui lòng chọn lồng thí nghiệm."); return; }
  ...
  if (error || !log) return (
    <main className="ml-64 mt-16 min-h-[calc(100vh-64px)] experiment-main-bg flex items-center justify-center">
      <div className="text-slate-500 text-center">
        ...
        <button type="button" onClick={() => navigate(-1)} className="mt-4 text-blue-600 hover:underline">Quay lại</button>
      </div>
    </main>
  );

  /* ─── Derived values (safe: log is guaranteed non-null below this line) ─── */
      ...
      <motion.div variants={staggerContainer} initial="hidden" animate="show" className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {["Completed", "Destroyed", "ExecutedBecauseOfDisease"].includes(normalizeStatus(log.status)) && (
              <>
                <button type="button" onClick={() => void downloadPdf("process")} disabled={exportingProcess} className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-slate-700 font-semibold hover:bg-slate-50 shadow-sm transition-all disabled:opacity-50">
                  <Download className="w-4 h-4" /> {exportingProcess ? "Đang xuất..." : "Nhật ký quy trình"}
                </button>
                <button type="button" onClick={() => void downloadPdf("summary")} disabled={exportingSummary} className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-slate-700 font-semibold hover:bg-slate-50 shadow-sm transition-all disabled:opacity-50">
                  <Download className="w-4 h-4" /> {exportingSummary ? "Đang xuất..." : "Báo cáo tổng kết"}
                </button>
              </>
            )}
            {isResearcher && normalizeStatus(log.status) === "InProgress" && isLastStage && allTasksCompleted && (
              <button type="button" onClick={() => setIsCompletionModalOpen(true)} className="btn-action-primary">
                <CheckCircle2 className="w-4 h-4" /> {t("experimentLog.completeExperiment") || "Hoàn thành"}
              </button>
            )}
            ...
            {samples.length > 0 && ((samples.filter(s => s.status === SampleStatus.ExecutedBecauseOfDisease).length / samples.length) * 100 > 50) && isResearcher && normalizeStatus(log.status) === "InProgress" && (
              <button type="button" onClick={() => setIsCancelModalOpen(true)} className="flex items-center gap-2 px-4 py-2 bg-rose-600 text-white rounded-xl font-semibold hover:bg-rose-700 shadow-sm transition-all">
                <Trash2 className="w-4 h-4" /> Hủy thí nghiệm
              </button>
            )}
          </div>
        </motion.div>
          ...
          <div className="space-y-6">
                  ...
                  <div className="flex justify-between items-center pb-3 border-b border-slate-100">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {normalizeStatus(log.status) === "WaitingForChangeStage" && (
                        <button type="button" onClick={openChangeStageModal} className="text-xs bg-blue-100 text-blue-700 px-3 py-1.5 rounded-lg font-semibold hover:bg-blue-200 transition-colors">
                          Chuyển GĐ
                        </button>
                      )}
                    </div>
                    ...
                    {log.seedling ? (
                      <div>
                        ...
                        {log.seedling.parentALocalName && (
                          <p className="text-sm text-slate-600 mt-1">Cây mẹ: {log.seedling.parentALocalName} {log.seedling.parentAScientificName && `(${log.seedling.parentAScientificName})`}</p>
                        )}
                      </div>
            ...
            {normalizeStatus(log.status) === "Completed" && (log.conclusion ?? log.issues ?? log.recommendations) && (
              <motion.div variants={fadeInUp} className="experiment-card p-6 md:p-8">
                <h2 className="text-lg font-bold text-blue-900 mb-4 flex items-center gap-2">
                  <ClipboardCheck className="w-5 h-5 text-blue-600" /> {t("experimentLog.completionDetails") || "Thông tương hoàn thành"}
                </h2>
            ...
            <motion.div variants={fadeInUp} className="experiment-card p-6 md:p-8">
              <h2 className="text-lg font-bold text-blue-900 mb-6 flex items-center gap-2">
                <Beaker className="w-5 h-5 text-blue-600" /> Hóa chất & Dụng cụ
                          ...
                          <ul className="space-y-2">
                            {items.map(c => (
                              <li key={c.id} className="flex items-center gap-2 text-sm text-slate-700 bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                                <span className="text-purple-500">•</span> <span className="font-medium">{c.name}</span> {c.concentrationUnit && <span className="text-slate-400">({c.concentrationUnit})</span>}
                              </li>
                            ))}
                          </ul>
                        </div>
                          ...
                          <ul className="space-y-2">
                            {items.map(m => (
                              <li key={m.id} className="flex items-center gap-2 text-sm text-slate-700 bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                                <span className="text-amber-500">•</span> <span className="font-medium">{m.name}</span> {m.unit && <span className="text-slate-400">({m.unit})</span>}
                              </li>
                            ))}
                          </ul>
                        </div>
                    ...
                    return (
                      <div key={stage.id} className={`p-5 rounded-2xl border transition-all ${isCurr ? "bg-blue-50 border-blue-600 shadow-sm" : "bg-slate-50 border-slate-200"}`}>
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          {isCurr && <span className="ml-auto text-[10px] font-bold bg-blue-600 text-white px-2 py-1 rounded-md uppercase">Hiện tại</span>}
                        </div>
                        {stage.stageDefinition?.description && <p className="text-xs text-slate-500 mb-3 line-clamp-2">{stage.stageDefinition.description}</p>}
                        <div className="flex flex-wrap gap-2 mt-auto">
                          <span className="text-[11px] font-semibold bg-white border border-slate-200 px-2 py-1 rounded-md text-slate-600">{stage.durationsDays} ngày</span>
                          <span className={`text-[11px] font-semibold px-2 py-1 rounded-md border flex items-center gap-1 ${stage.isSampleGenerated ? "bg-emerald-100 text-emerald-700 border-emerald-200" : "bg-slate-100 text-slate-600 border-slate-200"}`}>
                            <Leaf className="w-3 h-3" /> {stage.isSampleGenerated ? "Sinh chồi" : "Không sinh chồi"}
                          </span>
                        </div>
                      </div>
                    );
            ...
            <motion.div variants={fadeInUp} className="experiment-card p-6 md:p-8">
              ...
              {incidentsLoading ? <div className="py-8 text-center text-blue-600 animate-pulse font-medium">{t("common.loadingData")}</div> :
                incidents.length === 0 ? <p className="text-center text-slate-500 py-8 italic">{t("diseaseIncident.noIncidents")}</p> : (
        ...
        {activeTab === "summary" && (
          <motion.div variants={fadeInUp} className="space-y-6">
            {summaryLoading ? <div className="py-20 text-center text-blue-600 animate-pulse font-medium text-lg">Đang tải báo cáo tổng kết...</div> :
              summaryError ? <div className="py-20 text-center text-rose-500 font-medium text-lg">{summaryError}</div> : summary && (
                <>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-blue-100 flex flex-col justify-center items-center text-center">
                      <p className="text-sm font-bold text-slate-500 uppercase mb-2">Tổng số mẫu</p>
                    ...
                    <div className="bg-blue-50 p-6 rounded-2xl shadow-sm border border-blue-200 flex flex-col justify-center items-center text-center">
                      <p className="text-sm font-bold text-blue-600 uppercase mb-2">Còn sống</p>
                    ...
                    <div className="bg-rose-50 p-6 rounded-2xl shadow-sm border border-rose-200 flex flex-col justify-center items-center text-center">
                      <p className="text-sm font-bold text-rose-700 uppercase mb-2">Nhiễm bệnh</p>
                    ...
                    <div className="bg-sky-50 p-6 rounded-2xl shadow-sm border border-sky-200 flex flex-col justify-center items-center text-center">
                      <p className="text-sm font-bold text-sky-700 uppercase mb-2">Tỉ lệ sống</p>
                  ...
                  <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-blue-100">
                    <h3 className="font-bold text-slate-800 mb-4 flex items-center justify-between">
                      ...
                      <span className="text-sm text-slate-500 font-medium">Mục tiêu: {summary.expectedSamples} mẫu</span>
                    </h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-blue-100 flex flex-col items-center">
                      <h3 className="font-bold text-slate-800 w-full mb-6 text-center">Phân bố Giai đoạn</h3>
                      ...
                      ) : <p className="text-slate-500 italic my-auto">Không có dữ liệu biểu đồ</p>}
                    </div>

                    <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-blue-100 flex flex-col">
                      <h3 className="font-bold text-slate-800 mb-6">Thống kê Phiếu theo dõi</h3>
                      <div className="space-y-4 my-auto">
                        <div className="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-100">
                          <span className="font-semibold text-slate-600">Tổng số phiếu</span>
                        ...
                        <div className="flex justify-between items-center p-4 bg-amber-50 rounded-xl border border-amber-100">
                          <span className="font-semibold text-amber-700">Chờ duyệt</span>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden relative z-10 border border-slate-100">
              <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <h3 className="text-lg font-bold text-slate-800">Xác nhận chuyển giai đoạn</h3>
              ...
              <div className="p-6 space-y-5">
                <div>
                  <label className={labelClass}>Lồng thí nghiệm (Batch) <span className="text-rose-500">*</span></label>
                  ...
                  {batchesLoading && <p className="text-xs text-blue-600 mt-2 flex items-center gap-1"><Loader2 className="w-3 h-3 animate-spin" /> Đang tải danh sách lồng...</p>}
                </div>
                <div>
                  <label className={labelClass}>Lý do (tuỳ chọn)</label>
                  <textarea value={changeReason} onChange={e => setChangeReason(e.target.value)} rows={3} placeholder="Nhập lý do nếu cần..." disabled={changingStage} className={`${inputClass} resize-none`} />
                </div>
                {changeStageError && <div className="p-3 bg-rose-50 text-rose-600 text-sm rounded-lg flex items-center gap-2"><AlertCircle className="w-4 h-4 flex-shrink-0" />{changeStageError}</div>}
              </div>
              <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
                <button type="button" onClick={closeChangeStageModal} disabled={changingStage} className="px-5 py-2 bg-white border border-slate-200 text-slate-600 rounded-xl font-semibold hover:bg-slate-50 transition-colors">Hủy</button>
                <button type="button" onClick={handleChangeStage} disabled={changingStage || selectedBatchId === ""} className="btn-action-primary disabled:opacity-50">
                  {changingStage ? <Loader2 className="w-4 h-4 animate-spin" /> : null} {changingStage ? "Đang chuyển..." : "Xác nhận chuyển"}
                </button>
              </div>
            </motion.div>
          </div>
        ...
        {reviewingIncident && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden relative z-10 border border-slate-100">
              ...
              <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
                <button type="button" onClick={closeReviewModal} className="px-5 py-2 bg-white border border-slate-200 text-slate-600 rounded-xl font-semibold hover:bg-slate-50 transition-colors">Hủy</button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden relative z-10 border border-slate-100">
              <div className="px-6 py-5 border-b border-blue-100 bg-blue-50 flex justify-between items-center">
                <h3 className="text-lg font-bold text-blue-900 flex items-center gap-2"><CheckCircle2 className="w-5 h-5" /> {t("experimentLog.completeExperiment")}</h3>
              ...
              <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                <div>
                  ...
                  <textarea value={completionConclusion} onChange={e => setCompletionConclusion(e.target.value)} rows={3} placeholder={t("experimentLog.conclusionPlaceholder") || "Nhập kết luận (tùy chọn)"} className={`${inputClass} resize-none`} />
                </div>
                <div>
                  <label className={labelClass}>{t("experimentLog.issues")}</label>
                  <textarea value={completionIssues} onChange={e => setCompletionIssues(e.target.value)} rows={3} placeholder={t("experimentLog.issuesPlaceholder") || "Nhập các vấn đề gặp phải (tùy chọn)"} className={`${inputClass} resize-none`} />
                </div>
                <div>
                  <label className={labelClass}>{t("experimentLog.recommendations")}</label>
                  <textarea value={completionRecommendations} onChange={e => setCompletionRecommendations(e.target.value)} rows={3} placeholder={t("experimentLog.recommendationsPlaceholder") || "Nhập các đề xuất (tùy chọn)"} className={`${inputClass} resize-none`} />
                </div>
              ...
              <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
                <button type="button" onClick={closeCompletionModal} className="px-5 py-2 bg-white border border-slate-200 text-slate-600 rounded-xl font-semibold hover:bg-slate-50 transition-colors">Hủy</button>
                <button type="button" onClick={handleSubmitCompletion} disabled={isSubmittingCompletion} className="btn-action-primary disabled:opacity-50">
                  {isSubmittingCompletion ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle2 className="w-4 h-4" />} Hoàn thành
                </button>
              </div>
            </motion.div>
          </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden relative z-10 border border-slate-100">
              <div className="px-6 py-5 border-b border-rose-100 bg-rose-50 flex justify-between items-center">
                <h3 className="text-lg font-bold text-rose-800 flex items-center gap-2"><Trash2 className="w-5 h-5" /> {t("experimentLog.cancelExperiment")}</h3>
              ...
              <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                <div className="bg-amber-50 p-4 rounded-xl text-amber-800 text-sm mb-2 border border-amber-200 flex items-start gap-3">
                  ...
                  <p>Hành động này sẽ hủy bỏ nhật ký thí nghiệm hiện tại. Vui lòng nhập đầy đủ thông tin báo cáo trước khi hủy.</p>
                </div>
                <div>
                  <label className={labelClass}>{t("common.reason")}</label>
                  <textarea value={cancelReason} onChange={e => setCancelReason(e.target.value)} rows={2} placeholder="Nhập lý do hủy" className={`${inputClass} resize-none`} />
                </div>
                <div>
                  <label className={labelClass}>{t("experimentLog.conclusion")}</label>
                  <textarea value={cancelConclusion} onChange={e => setCancelConclusion(e.target.value)} rows={2} placeholder={t("experimentLog.conclusionPlaceholder") || "Nhập kết luận (tùy chọn)"} className={`${inputClass} resize-none`} />
                </div>
                <div>
                  <label className={labelClass}>{t("experimentLog.issues")}</label>
                  <textarea value={cancelIssue} onChange={e => setCancelIssue(e.target.value)} rows={2} placeholder={t("experimentLog.issuesPlaceholder") || "Nhập các vấn đề gặp phải (tùy chọn)"} className={`${inputClass} resize-none`} />
                </div>
                <div>
                  <label className={labelClass}>{t("experimentLog.recommendations")}</label>
                  <textarea value={cancelRecommendation} onChange={e => setCancelRecommendation(e.target.value)} rows={2} placeholder={t("experimentLog.recommendationsPlaceholder") || "Nhập các đề xuất (tùy chọn)"} className={`${inputClass} resize-none`} />
                </div>
              </div>
              <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
                <button type="button" onClick={closeCancelModal} className="px-5 py-2 bg-white border border-slate-200 text-slate-600 rounded-xl font-semibold hover:bg-slate-50 transition-colors">Đóng</button>
                <button type="button" onClick={handleCancelExperiment} disabled={cancelLoading} className="px-5 py-2 bg-rose-600 text-white rounded-xl font-semibold hover:bg-rose-700 transition-colors flex items-center gap-2 disabled:opacity-50">
                  {cancelLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4" />} Hủy Thí Nghiệm
                </button>
              </div>
            </motion.div>
          </div>
