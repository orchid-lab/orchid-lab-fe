...
const ExperimentLogDetail = () => {
  ...
  const [completionRecommendations, setCompletionRecommendations] =
    useState("");
  ...
  const handleChangeStage = async () => {
    if (!id || selectedBatchId === "") {
      setChangeStageError(t("experimentLog.selectBatchRequired"));
      return;
    }
    ...
    try {
      ...
      const axiosError = e as AxiosError<ApiErrorResponse>;
      setChangeStageError(
        axiosError.response?.data?.detail?.trim() ??
          axiosError.response?.data?.title?.trim() ??
  ...
  useEffect(() => {
    ...
    axiosInstance
      ...
      .then((res) => {
        ...
        setLog({
          ...anyLog,
          createdDate: anyLog.createdDate ?? anyLog.create_date,
        } as ExperimentLogDetailType);
      })
  ...
  useEffect(() => {
    ...
    if (tcbId) {
      axiosInstance
        .get(`/api/batches/${tcbId}`)
        .then((res) => {
          const raw = res.data;
          setLabName(
            (raw?.value?.labName as string) ??
      ...
      >
        ...
        >
          <div className="flex items-center gap-4">
            <button
              type="button"
              onClick={() => navigate("/researcher/experiment-log")}
              className="p-2 border border-slate-300 rounded-xl text-slate-500 hover:text-blue-600 hover:border-blue-600 hover:bg-blue-50 transition-all bg-white"
            >
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {["Completed", "Destroyed", "ExecutedBecauseOfDisease"].includes(
              ...
              <>
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-slate-700 font-semibold hover:bg-slate-50 shadow-sm transition-all disabled:opacity-50"
                >
                  <Download className="w-4 h-4" />
                  {exportingProcess
                    ? t("experimentLog.exporting")
                    : t("experimentLog.exportProcess")}
                </button>
                <button
                  type="button"
            ...
            {samples.length > 0 &&
              ...
              normalizeStatus(log.status) === "InProgress" && (
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-rose-600 text-white rounded-xl font-semibold hover:bg-rose-700 shadow-sm transition-all"
                >
        ...
        {normalizeStatus(log.status) === "WaitingForChangeStage" && (
          <motion.div
            variants={fadeInUp}
            className="relative overflow-hidden rounded-2xl border-2 border-blue-400 bg-gradient-to-r from-blue-600 to-blue-500 shadow-lg shadow-blue-200"
          >
            <div
              className="absolute inset-0 opacity-10"
              style={{
                backgroundImage:
                  "repeating-linear-gradient(45deg, white 0px, white 1px, transparent 0px, transparent 50%)",
        ...
        <motion.div
          variants={fadeInUp}
          className="flex gap-2 border-b border-blue-100"
        >
          <button
            type="button"
            onClick={() => setActiveTab("detail")}
            className={`experiment-tab ${activeTab === "detail" ? "experiment-tab-active" : "experiment-tab-inactive"}`}
          >
          ...
          <button
            ...
            className={`experiment-tab ${activeTab === "summary" ? "experiment-tab-active" : "experiment-tab-inactive"}`}
          >
        ...
        {activeTab === "detail" && (
          <div className="space-y-6">
            ...
            <motion.div variants={fadeInUp} className="experiment-card">
              <div className="p-6 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  <div className="flex justify-between items-center pb-3 border-b border-slate-100">
                    ...
                    <span className="text-slate-800 font-medium">
                      {log.method?.name ?? log.methodName ?? "-"}
                    </span>
                  </div>
                  <div className="flex justify-between items-center pb-3 border-b border-slate-100">
                    <span className="text-sm text-slate-500 font-semibold uppercase">
                  ...
                  <div className="flex justify-between items-center pb-3 border-b border-slate-100">
                    ...
                    <span className={getSampleBadge(log.status).className}>
                      {getSampleBadge(log.status).label}
                    </span>
                  </div>
                  <div className="flex justify-between items-center pb-3 border-b border-slate-100">
                    <span className="text-sm text-slate-500 font-semibold uppercase">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {normalizeStatus(log.status) ===
                        ...
                        <button
                          type="button"
                          onClick={openChangeStageModal}
                          className="text-xs bg-blue-600 text-white px-3 py-1.5 rounded-lg font-semibold hover:bg-blue-700 transition-colors flex items-center gap-1.5 shadow-sm"
                        >
                ...
                <div className="space-y-6">
                  {log.notes && (
                    <div>
                      <span className="text-sm text-slate-500 font-semibold uppercase mb-1 block">
            ...
            {normalizeStatus(log.status) === "Completed" &&
              (log.conclusion ?? log.issues ?? log.recommendations) && (
                <motion.div
                  variants={fadeInUp}
                  className="experiment-card p-6 md:p-8"
                >
                  <h2 className="text-lg font-bold text-blue-900 mb-4 flex items-center gap-2">
                    <ClipboardCheck className="w-5 h-5 text-blue-600" />{" "}
                    {t("experimentLog.completionDetails")}
                  </h2>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {log.conclusion && (
                      <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-xl">
                        <p className="text-xs font-semibold text-emerald-700 uppercase mb-2">
                          {t("experimentLog.conclusion")}
                        </p>
                    ...
                    {log.recommendations && (
                      <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl">
                        <p className="text-xs font-semibold text-blue-700 uppercase mb-2">
                          {t("experimentLog.recommendations")}
                        </p>
                        <p className="text-sm text-slate-800 whitespace-pre-wrap">
                          {log.recommendations}
                        </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div>
                  <h3 className="text-md font-semibold text-purple-800 mb-4 bg-purple-50 p-3 rounded-xl flex items-center gap-2">
                    <span className="bg-purple-200 text-purple-700 w-6 h-6 flex items-center justify-center rounded-md text-xs">
                      🧪
                    </span>
                          ...
                          <div key={cat}>
                            ...
                            <ul className="space-y-2">
                              {items.map((c) => (
                                <li
                                  key={c.id}
                                  className="flex items-center gap-2 text-sm text-slate-700 bg-slate-50 p-2.5 rounded-lg border border-slate-100"
                                >
                ...
                <div>
                  <h3 className="text-md font-semibold text-amber-800 mb-4 bg-amber-50 p-3 rounded-xl flex items-center gap-2">
                    <span className="bg-amber-200 text-amber-700 w-6 h-6 flex items-center justify-center rounded-md text-xs">
                      🔧
                    </span>
                  ...
                  {log.method.methodStages
                    ...
                    .map((stage) => {
                      ...
                      return (
                        <div
                          key={stage.id}
                          className={`p-5 rounded-2xl border transition-all ${isCurr ? "bg-blue-50 border-blue-600 shadow-sm" : "bg-slate-50 border-slate-200"}`}
                        >
                          <div className="flex items-center gap-3 mb-2">
                            <span
                              className={`w-6 h-6 flex items-center justify-center rounded-full text-xs font-bold ${isCurr ? "bg-blue-600 text-white" : "bg-slate-300 text-slate-600"}`}
                            >
                              {stage.order}
                            </span>
                            <span
                              className={`font-bold ${isCurr ? "text-blue-900" : "text-slate-700"}`}
                            >
                          ...
                          <div className="flex flex-wrap gap-2 mt-auto">
                            ...
                            <span
                              className={`text-[11px] font-semibold px-2 py-1 rounded-md border flex items-center gap-1 ${stage.isSampleGenerated ? "bg-emerald-100 text-emerald-700 border-emerald-200" : "bg-slate-100 text-slate-600 border-slate-200"}`}
                            >
            ...
            >
              <h2 className="text-lg font-bold text-blue-900 mb-6 flex items-center gap-2">
                ...
                {t("experimentLog.sampleList")} ({samples.length})
              </h2>
              {samples.length === 0 ? (
                <p className="text-center text-slate-500 py-8 italic">
                  {t("experimentLog.noSamples")}
                </p>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {samples.map((s) => (
                    <div
                      key={s.id}
                      onClick={() => navigate(`/researcher/samples/${s.id}`)}
                      className="p-4 bg-white border border-slate-200 hover:border-blue-600 hover:shadow-md transition-all rounded-xl cursor-pointer group"
                    >
                      <p className="font-bold text-slate-800 group-hover:text-blue-600 transition-colors truncate">
                        {s.name}
                      </p>
                      <p className="text-xs text-slate-500 mt-1 mb-3 truncate">
                        {s.description ?? "-"}
                      </p>
                      <div className="flex justify-between items-center mt-auto">
                        ...
                        <span
                          className={
                            getSampleBadge(s.status ?? s.statusEnum).className
                          }
                        >
                          {getSampleBadge(s.status ?? s.statusEnum).label}
                        </span>
                      </div>
                    </div>
                  ))}
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                ...
                <div className="flex items-center gap-3">
                  <select
                    ...
                    onChange={(e) =>
                      setIncidentStatusFilter(
                        e.target.value === ""
                          ? undefined
                          : (e.target.value as DiseaseIncidentStatus),
                      )
                    }
                    className="border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-600/20 outline-none"
                  >
                  ...
                  <button
                    ...
                    className="bg-slate-100 text-slate-700 hover:bg-slate-200 px-4 py-2 rounded-lg text-sm font-semibold transition-colors disabled:opacity-50"
                  >
                    {incidentsLoading ? (
              ...
              {incidentsLoading ? (
                <div className="py-8 text-center text-blue-600 animate-pulse font-medium">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {incidents.map((inc) => (
                        ...
                        >
                          ...
                          <td className="px-5 py-3 text-center">
                            <span
                              className={`inline-flex px-2.5 py-1 rounded-full text-[11px] font-bold border ${getIncidentStatusConfig(inc.status).class}`}
                            >
                          ...
                          <td className="px-5 py-3 text-center">
                            {(inc.status === DiseaseIncidentStatus.AIDetected ||
                              ...
                              <button
                                ...
                                className="bg-blue-50 text-blue-700 hover:bg-blue-100 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors"
                              >
        ...
        {activeTab === "summary" && (
          <motion.div variants={fadeInUp} className="space-y-6">
            {summaryLoading ? (
              ...
              <div className="py-20 text-center text-rose-500 font-medium text-lg">
                {summaryError}
              </div>
            ) : (
              summary && (
                <>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="bg-rose-50 p-6 rounded-2xl shadow-sm border border-rose-200 flex flex-col justify-center items-center text-center">
                      <p className="text-sm font-bold text-rose-700 uppercase mb-2">
                        {t("experimentLog.infectedSamples")}
                      </p>
                    ...
                    <div className="bg-sky-50 p-6 rounded-2xl shadow-sm border border-sky-200 flex flex-col justify-center items-center text-center">
                      <p className="text-sm font-bold text-sky-700 uppercase mb-2">
                        {t("experimentLog.survivalRateLabel")}
                      </p>
                  ...
                  <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-blue-100">
                    <h3 className="font-bold text-slate-800 mb-4 flex items-center justify-between">
                      ...
                      <span className="text-sm text-slate-500 font-medium">
                        {t("experimentLog.targetLabel")}:{" "}
                        {summary.expectedSamples} {t("experimentLog.samples")}
                      </span>
                    </h3>
                    <div className="w-full bg-slate-100 rounded-full h-4 mb-2 overflow-hidden shadow-inner">
                      <div
                        className="bg-blue-600 h-4 rounded-full transition-all duration-1000 relative"
                        style={{
                    ...
                    <p className="text-right text-sm font-bold text-blue-600">
                      {summary.progressRate.toFixed(1)}%
                    </p>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-blue-100 flex flex-col items-center">
                      <h3 className="font-bold text-slate-800 w-full mb-6 text-center">
                        {t("experimentLog.stageDistribution")}
                      </h3>
                      {summary.stageDistribution.length > 0 ? (
                        ...
                        <p className="text-slate-500 italic my-auto">
                          {t("common.noData")}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-blue-100 flex flex-col">
                      <h3 className="font-bold text-slate-800 mb-6">
                        {t("experimentLog.monitoringStats")}
                      </h3>
                      <div className="space-y-4 my-auto">
                        <div className="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-100">
                          <span className="font-semibold text-slate-600">
                            {t("experimentLog.totalMonitoringLogs")}
                          </span>
            ...
            >
              <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <h3 className="text-lg font-bold text-slate-800">
                  {t("experimentLog.confirmChangeStageTitle")}
                </h3>
                <button
                  type="button"
                  onClick={closeChangeStageModal}
                  ...
                  <div className="relative">
                    ...
                    >
                      <span>
                        {selectedBatchId === ""
                          ? batchesLoading
                            ? t("experimentLog.loadingBatches")
                            : t("experimentLog.selectBatchPlaceholder")
                          : getBatchOptionLabel(
                              readyBatches.find(
                                (b) => b.id === selectedBatchId,
                              ) ?? {
                                id: selectedBatchId as number,
                                batchName: String(selectedBatchId),
                              },
                            )}
                      </span>
                      <div className="text-slate-400">
                        {batchesLoading ? (
                          ...
                          <ChevronDown
                            className={`w-5 h-5 transition-transform ${isBatchDropdownOpen ? "rotate-180" : ""}`}
                          />
                        )}
                      </div>
                    </button>
                  ...
                  {!batchesLoading &&
                    ...
                    readyBatches.length === 0 && (
                      <p className="text-xs text-amber-600 mt-2 flex items-center gap-1">
                        ...
                        {t("experimentLog.noBatchesAvailable")}
                      </p>
                    )}
                </div>
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder={t(
                      "experimentLog.changeStageReasonPlaceholder",
                    )}
              ...
              <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
                <button
                  ...
                  onClick={closeChangeStageModal}
                  disabled={changingStage}
                  className="px-5 py-2 bg-white border border-slate-200 text-slate-600 rounded-xl font-semibold hover:bg-slate-50 transition-colors"
                >
                  {t("common.cancel")}
                </button>
                <button
                  ...
                  disabled={
                    changingStage || selectedBatchId === "" || batchesLoading
                  }
                  className="btn-action-primary disabled:opacity-50"
                >
                  {changingStage ? (
                    <Loader2 className="w-4 h-4 animate-spin" />
                  ) : null}
                  {changingStage
                    ? t("experimentLog.changingStage")
                    : t("experimentLog.confirmChangeStageBtn")}
                </button>
              </div>
            </motion.div>
          </div>
              ...
              <div className="p-6 space-y-5">
                ...
                <div>
                  ...
                  <div className="flex gap-4 mt-2">
                    <label
                      className={`flex-1 p-4 rounded-xl border cursor-pointer transition-all flex items-center gap-3 ${reviewIsConfirmed ? "bg-blue-50 border-blue-600" : "bg-white border-slate-200 hover:border-blue-600/50"}`}
                    >
                      <input
                        type="radio"
                        name="review"
                      ...
                      />
                      <span
                        className={`font-semibold ${reviewIsConfirmed ? "text-blue-900" : "text-slate-600"}`}
                      >
                        {t("diseaseIncident.reviewConfirm")}
                      </span>
                    </label>
                    <label
                      className={`flex-1 p-4 rounded-xl border cursor-pointer transition-all flex items-center gap-3 ${!reviewIsConfirmed ? "bg-slate-100 border-slate-400" : "bg-white border-slate-200 hover:border-slate-400"}`}
                    >
                      <input
                        type="radio"
                        name="review"
                      ...
                      />
                      <span
                        className={`font-semibold ${!reviewIsConfirmed ? "text-slate-800" : "text-slate-600"}`}
                      >
              ...
              <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder={t("experimentLog.conclusionPlaceholder")}
                    className={`${inputClass} resize-none`}
                  />
                </div>
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder={t("experimentLog.issuesPlaceholder")}
                    className={`${inputClass} resize-none`}
                  />
                </div>
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setCompletionRecommendations(e.target.value)
                    }
                    rows={3}
                    placeholder={t("experimentLog.recommendationsPlaceholder")}
                    className={`${inputClass} resize-none`}
                  />
                </div>
                {completionError && (
            ...
            >
              ...
              <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    placeholder={t("experimentLog.cancelReason")}
                    className={`${inputClass} resize-none`}
                  />
                </div>
                <div>
                  ...
                  <textarea
                    value={cancelConclusion}
                    onChange={(e) => setCancelConclusion(e.target.value)}
                    rows={2}
                    placeholder={t("experimentLog.conclusionPlaceholder")}
                    className={`${inputClass} resize-none`}
                  />
                ...
                <div>
                  ...
                  <textarea
                    value={cancelRecommendation}
                    onChange={(e) => setCancelRecommendation(e.target.value)}
                    rows={2}
                    placeholder={t("experimentLog.recommendationsPlaceholder")}
                    className={`${inputClass} resize-none`}
                  />
              ...
              <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
                ...
                <button
                  ...
                  className="px-5 py-2 bg-rose-600 text-white rounded-xl font-semibold hover:bg-rose-700 transition-colors flex items-center gap-2 disabled:opacity-50"
                >
                  {cancelLoading ? (
                    <Loader2 className="w-4 h-4 animate-spin" />
                  ) : (
