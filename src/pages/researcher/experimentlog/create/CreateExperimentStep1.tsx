...
const CreateExperimentStep1 = () => {
  ...
  useEffect(() => {
    if (DEV_OFFLINE) {
      setMethods([{ id: "1", name: "Nuôi cấy mô tế bào (Invitro)", totalDurationDays: 97 }]);
      return;
    }
        ...
        setMethods(
          Array.isArray(raw.data)
            ? raw.data.map((m) => ({
                ...
                totalDurationDays: m.totalDurationDays as number,
              }))
            : []
        );
      })
  ...
  // Auto-calculate endDate from startDate + method.totalDurationDays
  useEffect(() => {
    if (!startDate || !selectedMethod) { setEndDate(""); return; }
    const methodObj = methods.find((m) => m.id === selectedMethod);
    if (!methodObj?.totalDurationDays) { setEndDate(""); return; }
    const end = new Date(startDate);
    end.setDate(end.getDate() + methodObj.totalDurationDays);
    setEndDate(end.toISOString().split("T")[0]);
  }, [startDate, selectedMethod, methods]);

  // Fetch detailed method info (stages)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    setForm((prev) => ({
      ...
      methodID: methodObj?.id ?? "", methodName: methodObj?.name ?? "", methodType: (methodObj as any)?.type ?? "",
          ...
          <div className="p-6 md:p-8 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className={`${inputClass} bg-slate-50 flex items-center min-h-[46px]`}>
                  {endDate ? (
                    <>
                      <span className="text-slate-700">
                        {new Date(endDate).toLocaleDateString("vi-VN", { day: "2-digit", month: "2-digit", year: "numeric" })}
                      </span>
                      {selectedMethodObj?.totalDurationDays && (
                        <span className="ml-auto text-xs text-[#2D5A27] font-medium bg-[#E4F0E8] px-2 py-0.5 rounded-full whitespace-nowrap">
                          +{selectedMethodObj.totalDurationDays} ngày
                        </span>
                      )}
                    </>
                  ) : (
            ...
            {selectedMethod && (
              <div className="bg-[#F4F7F4] p-5 rounded-xl border border-[#DDEEE0]">
                <h3 className="font-bold text-[#1e3e1c] flex items-center gap-2 mb-3">
                  ...
                  {t("experimentLog.methodDetail")}: {selectedMethodObj?.name}
                </h3>
                <div className="text-sm text-slate-600 space-y-3 pl-6">
                  <p><strong>{t("experimentLog.description")}:</strong> {selectedMethodObj?.description}</p>
                  {selectedMethodObj?.totalDurationDays && (
                    <p>
                      <strong>Tổng thời gian:</strong>{" "}
                      <span className="text-[#2D5A27] font-semibold">{selectedMethodObj.totalDurationDays} ngày</span>
                    </p>
                  )}
                  <div>
