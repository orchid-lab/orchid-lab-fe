...
export default function AdminMethod() {
  ...
  const tabs: { key: TabKey; label: string; icon: React.ReactNode }[] = [
    {
      key: "list",
      label: t("method.methodManagement") || "Danh sách",
      icon: <Layers className="w-4 h-4" />,
    },
    {
      key: "stats",
      label: t("seedling.successRateAnalysis") || "Tỉ lệ thành công",
    ...
    <main className="admin-method-page ml-64 mt-16 min-h-[calc(100vh-64px)] bg-[#fffbfb] text-slate-900">
      ...
      <div className="p-6 space-y-6">
        ...
        <motion.div
          ...
          animate="visible"
          custom={0.5}
          className="flex gap-1 bg-white/80 backdrop-blur-sm border border-rose-100 rounded-2xl shadow-sm p-1.5 w-fit"
        >
          {tabs.map((tab) => (
            <motion.button
              key={tab.key}
              ...
              className={`flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-medium transition-all ${
                activeTab === tab.key
                  ...
                  : "text-slate-500 hover:text-[#9f1239] hover:bg-rose-50"
              }`}
            >
              {tab.icon}
              {tab.label}
