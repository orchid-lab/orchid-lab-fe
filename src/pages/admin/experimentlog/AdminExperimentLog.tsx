...
const AdminExperimentLog = () => {
  ...
  const fetchSampleCount = async (experimentLogId: string): Promise<number> => {
    try {
      const response = await axiosInstance.get(
        `/api/sample?pageNo=1&pageSize=1000&experimentLogId=${experimentLogId}`,
      );
  ...
  const fetchStatsOnly = useCallback(async () => {
    try {
      const res = await axiosInstance.get(
        "/api/experimentlog?pageNumber=1&pageSize=1000",
      );
      const data = res.data;
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const res = await axiosInstance.get(
          `/api/experimentlog?${params.toString()}`,
        );
        const data = res.data;
  ...
  return (
    <main className="ml-64 mt-16 min-h-[calc(100vh-64px)] bg-gray-50 ">
      ...
      <div className="bg-white shadow-lg">
        <div className="px-6 py-4">
          ...
          <div className="flex justify-center">
            <div className="mb-6 flex justify-center items-center mr-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 mb-3 mt-6">
        <div className="rounded-xl border bg-blue-50 border-blue-200 px-6 py-4 flex flex-col items-center animate-scale-in hover-lift stagger-1">
          <span className="text-sm text-gray-600 mb-2 font-medium">
            {t("experimentLog.totalExperiments")}
          </span>
          <span className="text-3xl font-bold text-blue-700 transition-all duration-300">
            {stats.total}
          </span>
        </div>
                        ...
                        <button
                          ...
                          className={`px-4 py-2 rounded-lg font-medium transition-all duration-200 ${
                            currentPage === pageNum
                              ...
                              : "bg-white border border-gray-300 hover:bg-green-50 hover:border-green-500 shadow-sm"
                          }`}
                        >
                          {pageNum}
                        </button>
