...
export default function AdminSeedlings() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      runProgress();
      try {
        const res = await axiosInstance.get("/api/seedlings?pageNumber=1&pageSize=1000");
        const json = res.data as SeedlingApiResponse;
        setAllSeedlings((json.data || []).reverse());
      } catch {
        setAllSeedlings([]);
      } finally {
        setLoading(false);
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const res = await axiosInstance.get("/api/user?pageNumber=1&pageSize=1000");
        const users = res.data?.data ?? [];
        const researchers = users
          .filter((u: User) => u.role === "Researcher")
          .map((u: User) => u.name);
                  ...
                  <AnimatePresence mode="popLayout">
                    {currentSeedlings.map((s, idx) => (
                      ...
                      >
                        ...
                        <td className="p-4 text-center text-gray-700">{s.parent1 ?? "-"}</td>
                        <td className="p-4 text-center text-gray-700 italic text-sm">{s.parent2 ?? "-"}</td>
