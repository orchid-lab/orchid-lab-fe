...
export default function AdminSeedlingDetail() {
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      setLoading(true);
      try {
        const res = await axiosInstance.get(`/api/seedling/${id}`);
  ...
  if (loading) {
    return (
      <main className="ml-64 mt-16 min-h-[calc(100vh-64px)] bg-gray-100 flex items-center justify-center">
        <div className="text-gray-500">{t("seedling.loading")}</div>
      </main>
    );
  }
  ...
  if (!seedling) {
    return (
      <main className="ml-64 mt-16 min-h-[calc(100vh-64px)] bg-gray-100 flex items-center justify-center">
        <div className="text-red-500">{t("seedling.notFound")}</div>
      </main>
    );
  }
  ...
  return (
    <main className="ml-0 sm:ml-64 mt-16 min-h-[calc(100vh-64px)] bg-gray-100 px-2 sm:px-4 md:px-8">
      ...
      <div className="text-gray-500 mb-4">{t("seedling.detailedInfo")}</div>
      <div className="bg-white rounded shadow p-4 sm:p-6 flex flex-col md:flex-row gap-4 md:gap-8">
        <div className="flex-1">
          <div className="mb-2">
            <span className="font-semibold">{t("seedling.seedlingName")}:</span>{" "}
            {seedling.localName}
          </div>
          <div className="mb-2">
            <span className="font-semibold">{t("seedling.scientificName")}:</span>{" "}
          ...
          <div className="mb-2">
            <span className="font-semibold">{t("seedling.dateOfBirth")}:</span>{" "}
            {seedling.doB}
          </div>
          <div className="mb-2">
          ...
          <div className="mb-2">
            ...
            {seedling.characteristics && seedling.characteristics.length > 0 ? (
              ...
              <span className="text-gray-400"> {t("seedling.noCharacteristics")}</span>
            )}
          </div>
        </div>
      </div>
